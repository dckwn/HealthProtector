...
public class MemberService {
	...
	private String saveDirectory = "/home/ubuntu/profile";
//	private String saveDirectory = "D:\\upload\\profile";
	...
	public int join(MemberDTO dto) throws ParseException {
	    ...
	    String filenameWithTime = currentTime + "_" + image;

	    DateFormat dateFormatBirth = new SimpleDateFormat("yyyy-MM-dd");
	    java.util.Date birth = dateFormatBirth.parse(dto.getStrbirth());
	    java.sql.Date sqlBirth = new java.sql.Date(birth.getTime());
	    dto.setBirth(sqlBirth);
	    
	    dto.setProfile_image(filenameWithTime);
	    
	    File f = new File(saveDirectory, filenameWithTime);
...
//		System.out.println(dto.getProfile_Upload().getSize());
		DateFormat dateFormat = new SimpleDateFormat("yyyyMMdd_HHmmss");
	    String currentTime = dateFormat.format(new java.util.Date());
	    
		if(dto.getProfile_Upload().getSize() == 0) {
